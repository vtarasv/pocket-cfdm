...
def _expansion(omega, eps, L=2000):  # the summation term only
    p = 0
    for l_ in range(L):
        p += (2 * l_ + 1) * np.exp(-l_ * (l_ + 1) * eps**2) * np.sin(omega * (l_ + 1 / 2)) / np.sin(omega / 2)
    return p
...
def _score(exp, omega, eps, L=2000):  # score of density over SO(3)
    d_sigma = 0
    for l_ in range(L):
        hi = np.sin(omega * (l_ + 1 / 2))
        dhi = (l_ + 1 / 2) * np.cos(omega * (l_ + 1 / 2))
        lo = np.sin(omega / 2)
        dlo = 1 / 2 * np.cos(omega / 2)
        d_sigma += (2 * l_ + 1) * np.exp(-l_ * (l_ + 1) * eps**2) * (lo * dhi - hi * dlo) / lo ** 2
    return d_sigma / exp


if (cache_path / '.so3_omegas_array2.npy').exists():
    _omegas_array = np.load(str(cache_path / '.so3_omegas_array2.npy'))
    _cdf_vals = np.load(str(cache_path / '.so3_cdf_vals2.npy'))
    _score_norms = np.load(str(cache_path / '.so3_score_norms2.npy'))
    _exp_score_norms = np.load(str(cache_path / '.so3_exp_score_norms2.npy'))
else:
    os.makedirs(cache_path, exist_ok=True)
    logger.info(f"precomputing and saving SO(3) distribution table to cache {str(cache_path.absolute())}")
    ...
    np.save(str(cache_path / '.so3_omegas_array2.npy'), _omegas_array)
    np.save(str(cache_path / '.so3_cdf_vals2.npy'), _cdf_vals)
    np.save(str(cache_path / '.so3_score_norms2.npy'), _score_norms)
    np.save(str(cache_path / '.so3_exp_score_norms2.npy'), _exp_score_norms)
